...
# TCP 클라이언트 설정 추가
TCP_SERVER_HOST = '172.30.1.43'
TCP_SERVER_PORT = 12344
...
MENU_DATA = [
    {"menu_id": 1, "name": "튀김덮밥", "description": "바삭한 튀김과 특제 소스", "price": 10000},
    {"menu_id": 2, "name": "장어덮밥", "description": "달콤한 장어구이와 특제 양념", "price": 18000},
    {"menu_id": 3, "name": "비빔밥", "description": "신선한 야채와 고소한 참기름", "price": 9000},
    {"menu_id": 4, "name": "국수", "description": "쫄깃한 면발과 시원한 국물", "price": 8000}
]
...
def update_menu_status_api(order_id, menu_name):
    try:
        data = request.get_json()
        ...
        if new_status == 'Completed':
            ...
            result = cursor.fetchone()
            if result[0] == result[1]:  # 모든 메뉴가 완료된 경우
                cursor.execute("""
                    UPDATE orders 
        ...
        return jsonify({
            'success': True,
            'message': f'{menu_name}의 상태가 {new_status}로 업데이트되었습니다.'
        })
