...
class TaskManager(Node):
    ...
    def call_receive_order_service(self, order_id):
        ...
        future.add_done_callback(lambda future: self.receive_order_response(future, order_id))
    def receive_order_response(self, future, order_id):
        try:
            response = future.result()
            details = response.details
            self.get_logger().info(f'Received order: {details}')
            for detail in details:
                self.order_queue.put((order_id, detail.order_detail_id, detail.menu_name, detail.table_num))
            if not self.is_cooking and not self.is_serving:
                self.process_queue()
        except Exception as e:
    ...
    def process_queue(self):
        ...
        order_id, order_detail_id, menu_name, table_num = self.order_queue.get()
